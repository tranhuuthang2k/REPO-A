...
export async function getServerSideProps(context) {
  const params_tag = context.params.tag;
  const res = await db
    .collection("blogs")
    .orderBy("createdAt", "desc")
    .limit(10)
  ...
  const blogs = res.docs.map((snap) => {
    if (snap.data().tags.includes(params_tag)) {
      return {
        ...snap.data(),
